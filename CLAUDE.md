...
| **Oddschecker** | keyless bookmaker odds for upcoming fixtures, incl. correct score and anytime scorer per player; server-rendered JSON in each match page; Cloudflare challenges Python's TLS fingerprint so the ingest fetches through `curl` with a cookie jar. **Collected** (`ingest/oddschecker.py`, 2026-09-16, found by the owner). |
| **OddsPortal** (owner, 2026-09-16) | historical match odds back many seasons, but the results and match pages are a JavaScript app: the match list comes from a signed `ajax-sport-country-tournament-archive` endpoint (404 to a plain client even with cookies and a Referer) and a real browser first mints an anti-bot proof (`ajax-esi/ab-proof-mint`, `api/auth-token`) before any odds load. Its match-level 1X2 and over/under history duplicates football-data.co.uk (already loaded for four seasons; `ODDS_WEIGHT` 0.5-1.0 is indistinguishable, so better 1X2 history would move nothing); its unique history — correct score, both-to-score — is per match page behind that proof, and it holds no player-prop history. **Not scraped.** The question it would settle (the market's own P(clean sheet) vs the Poisson zero) is answered forward by the Oddschecker archive in ~8 gameweeks at no cost. |
